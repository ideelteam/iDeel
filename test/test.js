...
//	this.timeout(15000);
	...
	it('The first entry in the array has known properties', function(){
	    expect(requestResult[0]).to.have.all.keys(['_id','city','description','jobID','salary','title','availability','companyName']);
		...
		expect(response.body).to.satisfy(
			function (body) {
				for (var i = 0; i < body.length; i++) {
					...
					expect(body[i]).to.have.property('companyName').that.is.a('string');
					expect(body[i]).to.have.property('city').that.is.a('string');
					expect(body[i]).to.have.property('salary').that.is.a('string');
					expect(body[i]).to.have.property('availability').that.is.a('boolean');
